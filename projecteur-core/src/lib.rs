...
//! and input mapping can therefore be tested without a display
//! server.
...
pub enum ParseDeviceError {
    ...
    UnknownBus(String),
    /// A command-line device did not contain valid vendor and product IDs.
    InvalidAdditionalDevice(String),
}
...
impl fmt::Display for ParseDeviceError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidAdditionalDevice(value) => {
                write!(formatter, "invalid vendor/product ID pair {value:?}")
            }
        }
    }
}

/// Parse `VENDOR:PRODUCT[:NAME]` accepted by `--additional-device`.
///
/// Additional devices use the generic USB presenter implementation, matching
/// the historical command-line behavior.
///
/// # Errors
///
/// Returns [`ParseDeviceError::InvalidAdditionalDevice`] for missing, zero, or
/// non-hexadecimal identifiers.
pub fn parse_additional_device(value: &str) -> Result<SupportedDevice, ParseDeviceError> {
    let mut fields = value.splitn(3, ':');
    let parse_id = |field: Option<&str>| {
        let field = field.ok_or_else(|| ParseDeviceError::InvalidAdditionalDevice(value.into()))?;
        u16::from_str_radix(
            field
                .strip_prefix("0x")
        ...
        .ok()
        .filter(|id| *id != 0)
        .ok_or_else(|| ParseDeviceError::InvalidAdditionalDevice(value.into()))
    };
    let vendor = parse_id(fields.next())?;
    let product = parse_id(fields.next())?;
    let name = fields.next().filter(|name| !name.is_empty()).map_or_else(
        || format!("Additional presenter ({vendor:04x}:{product:04x})"),
        str::to_owned,
    );
    Ok(SupportedDevice {
        id: DeviceId {
...
mod tests {
    ...
    #[test]
    fn parses_additional_device_ids_and_optional_name() {
        let device = parse_additional_device("0x04b3:310c:Demo remote").unwrap();
        assert_eq!(device.id.vendor, 0x04b3);
        assert_eq!(device.id.product, 0x310c);
        assert_eq!(device.name, "Demo remote");
        assert!(parse_additional_device("04b3").is_err());
        assert!(parse_additional_device("0:310c").is_err());
    }
}
