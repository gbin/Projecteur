...
use crate::{Bus, DeviceId, SupportedDevice, all_supported_devices};
...
/// Discover built-in presenters plus additional generic USB devices.
///
/// # Errors
///
/// Returns [`DeviceScanError`] when `/sys/bus/hid/devices` cannot be listed.
pub fn scan_devices_with(
    additional_devices: &[SupportedDevice],
) -> Result<Vec<DiscoveredDevice>, DeviceScanError> {
    scan_devices_below_with(
...
pub fn scan_devices_below(
    ...
    scan_devices_below_with(hid_root, device_root, &[])
}

/// Discover devices below explicit roots with extra accepted device IDs.
///
/// # Errors
///
/// Returns [`DeviceScanError`] when `hid_root` cannot be listed.
pub fn scan_devices_below_with(
    hid_root: &Path,
    device_root: &Path,
    additional_devices: &[SupportedDevice],
) -> Result<Vec<DiscoveredDevice>, DeviceScanError> {
    ...
    let mut supported = all_supported_devices();
    supported.extend_from_slice(additional_devices);
